...
public abstract class RandomUtils {
	...
	public static String getCompanyMark() {
		StringBuffer str = new StringBuffer();
		str.append(CHARS.charAt((int) (Math.random() * 24))).append(
				getRandomNum(5));
		return str.toString();
	}
	...
	public static String getRandomStr(int length) {
		...
		for (int i = 0; i < length; i++) {
			randomBytes[i] = ALPHA_NUMERIC.charAt(getRandomInt(0,
					ALPHA_NUMERIC.length() - 1));
		}
...
//		System.out.println(CHARS.charAt((int) (Math.random() * 24)));
//
